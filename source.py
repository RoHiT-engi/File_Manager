...
def loggin():
    ...
    Button(login_frame, text="Enter", command=logging).grid(row=3, column=1)
...
def window31():
    ...
    def working(take_path):
        ...
        Button(work_on_file, text="sort File by extention").grid(row=0, column=0)
        Button(work_on_file, text="compress using ZIP File", command=lambda: zip_fun1(take_path)).grid(row=1, column=0)
        Button(work_on_file, text="open a file").grid(row=2, column=0)
        Button(work_on_file, text="Search a file").grid(row=0, column=1)
        Button(work_on_file, text="Sort a File by filetype").grid(row=1, column=1)
        Button(work_on_file, text="Make a secure Folder",command=lambda :secure_folder(take_path)).grid(row=2, column=1)
        Button(work_on_file, text="delete a file").grid(row=3, column=0)
        Button(work_on_file, text="list all files").grid(row=3, column=1)
        Button(work_on_file, text="copy files to another dirc").grid(row=4, column=0)
        Button(work_on_file, text="move files to another dirc").grid(row=4, column=1)
        Button(work_on_file, text="share directory on LAN").grid(row=5, column=0)
